...
public class AutoPathArcShotBehaviour extends Behaviour {
  ...
  public AutoPathArcShotBehaviour(
      int priority,
  ...
  private Command buildShootReadyCommand(BehaviourContext ctx) {
    return Commands.waitSeconds(2).andThen(Commands.runOnce(() -> pieceCount.set(0L)));
  }
  ...
  private boolean isReadyToShoot(
    ...
    return nearPose(robotPose, goalPose, 0.32, 12.0);
  }

  private static Pose2d orbitGoalPose(
      Pose2d robotPose, Pose2d desiredGoal, Pose2d targetPose, double maxAngStepRad) {
    if (robotPose == null || desiredGoal == null || targetPose == null) return desiredGoal;

    var c = targetPose.getTranslation();
    var g = desiredGoal.getTranslation();
    var r0 = g.getDistance(c);
    if (!Double.isFinite(r0) || r0 < 0.2) return desiredGoal;

    double r = Math.max(0.8, Math.min(7.0, r0));

    double aCur = Math.atan2(robotPose.getY() - c.getY(), robotPose.getX() - c.getX());
    double aDes = Math.atan2(g.getY() - c.getY(), g.getX() - c.getX());
    double da = MathUtil.angleModulus(aDes - aCur);
    double step = Math.max(-maxAngStepRad, Math.min(maxAngStepRad, da));
    double aNext = aCur + step;

    double x = c.getX() + Math.cos(aNext) * r;
    double y = c.getY() + Math.sin(aNext) * r;

    Rotation2d rot = desiredGoal.getRotation();
  ...
  public Command build(BehaviourContext ctx) {
    ...
    final double ORBIT_ENABLE_DIST_M = 4.2;
    final double ORBIT_MAX_ANG_STEP_RAD = Math.toRadians(10.0);

    final double LOOKAHEAD_S = 0.24;
    ...
    Consumer<Boolean> finalizeEpisode =
        forceSuccess -> {
          if (lastEpisodeGoal.get() == null) return;
          long now = System.nanoTime();
          long last = lastEpisodeFinalizeNs.get();
          if (last != 0L && now - last < EP_COOLDOWN_NS) return;

          boolean success = forceSuccess;
          if (episodeEverNearGoal.get()) success = true;
    ...
    return Commands.run(
            () -> {
              ...
              if (prevCat != null && prevCat != cat) {
                if (lastEpisodeGoal.get() != null) finalizeEpisode.accept(false);
                ctx.planner.clearCommitted();
                lastActive.set(null);
                timingStartNs.set(0);
                timingStation.set(null);
                if (shootReadyCmd != null && shootReadyCmd.isScheduled()) shootReadyCmd.cancel();
              ...
              Pose2d tgt = goalPose;
              boolean scoring = cat == CategorySpec.kScore && piece;

              Pose2d plannerGoalPose = goalPose;
              if (scoring && tgt != null) {
                double distGoal = robotPose.getTranslation().getDistance(goalPose.getTranslation());
                if (distGoal <= ORBIT_ENABLE_DIST_M) {
                  plannerGoalPose = orbitGoalPose(robotPose, goalPose, tgt, ORBIT_MAX_ANG_STEP_RAD);
                }
              }

              ctx.repulsor.setCurrentGoal(sp);
              ctx.planner.setGoal(plannerGoalPose);

              double distToGoal =
                  robotPose.getTranslation().getDistance(plannerGoalPose.getTranslation());
              ...
              Pose2d lookaheadPose =
                  robotPose.exp(
                      new Twist2d(
                          speeds.vxMetersPerSecond * LOOKAHEAD_S,
                          speeds.vyMetersPerSecond * LOOKAHEAD_S,
                          speeds.omegaRadiansPerSecond * LOOKAHEAD_S));

              boolean readyToShoot = isReadyToShoot(lookaheadPose, goalPose, piece, cat);
              if (readyToShoot) {
                if (!shootLatched.get()) {
              ...
              Logger.recordOutput("autopatharc_goal_x", plannerGoalPose.getX());
              Logger.recordOutput("autopatharc_goal_y", plannerGoalPose.getY());
              Logger.recordOutput(
                  "autopatharc_goal_theta", plannerGoalPose.getRotation().getRadians());
              Logger.recordOutput("autopatharc_dist_to_goal", distToGoal);
              Logger.recordOutput("autopatharc_shoot_lock", shootGoalLocked.get());
        ...
        .finallyDo(
            interrupted -> {
              if (shootReadyCmd != null && shootReadyCmd.isScheduled()) shootReadyCmd.cancel();
              if (lastEpisodeGoal.get() != null) finalizeEpisode.accept(!interrupted);
              ctx.drive.runVelocity(new ChassisSpeeds());
            });
  }
